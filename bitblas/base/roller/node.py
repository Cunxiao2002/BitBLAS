    ...
    def footprint(self, shape, rstep, stride_map: Optional[Dict] = None) -> int:
        ...
        for block in self.blocks:
            ...
            for buffer in self.block_analyzer.get_input_buffers(block):
                ...
                # allocated by previous node, not counted here
                args_name = [arg.name for arg in self.args]
                buffer_name = buffer.name.replace("_reindex", "") 
                if buffer_name in args_name:
                    input_id = args_name.index(buffer_name)
