...
@tvm.ir.transform.module_pass(opt_level=0, name="FuseIdenticalLoops")
class FuseIdenticalLoops:
    def transform_module(self, mod, ctx):
        ...
        for gv, func in mod.functions.items():
            if gv.name_hint == func_name:
                new_func = func.with_attr("private", False)
        ...
        # func = mod[func_name]
        func = func_temp
        assert func != None, "the func doesn't occur"
        
        # Extract the body of the function
        body = func.body
        
        # Find the two loop nests
        loop_nests = []
        
        def find_loop_nests(stmt):
            if isinstance(stmt, tir.For) and tvm.tir.all(
                stmt.thread_binding.thread_tag == "blockIdx.z"
            ):
                loop_nests.append(stmt)
                return
            ...
            if isinstance(stmt, tir.SeqStmt):
                ...
                find_loop_nests(stmt.body)
            elif isinstance(stmt, tir.Block):
                find_loop_nests(stmt.body)
            elif isinstance(stmt, tir.BlockRealize):
                find_loop_nests(stmt.block)
        
        find_loop_nests(body)
        
        if len(loop_nests) < 2:
            print(f"Found only {len(loop_nests)} loop nests, need at least 2")
            return mod
        
        # Get the first two loop nests
        first_loop = loop_nests[0]
        second_loop = loop_nests[1]
        
        # Create a merged loop
        merged_loop = tir.For(
            first_loop.loop_var,
            first_loop.min,
            first_loop.extent,
            first_loop.kind,
            tir.SeqStmt([first_loop.body, second_loop.body]),
            first_loop.thread_binding,
            first_loop.annotations
        )
        
        # Replace the loops in the function body
        def replace_loops(stmt):
            if stmt == first_loop:
                return merged_loop
            elif stmt == second_loop:
                # Return an empty statement to effectively remove the second loop
            ...
            if isinstance(stmt, tir.For):
                return tir.For(
                    ...
                    stmt.extent,
                    stmt.kind,
                    replace_loops(stmt.body),
                    stmt.thread_binding,
                    stmt.annotations
                )
            elif isinstance(stmt, tir.SeqStmt):
                return tir.SeqStmt([replace_loops(s) for s in stmt.seq])
            elif isinstance(stmt, tir.Block):
                return tir.Block(
                    stmt.iter_vars,
                    stmt.reads,
                ...
                return tir.BlockRealize(
                    stmt.iter_values,
                    stmt.predicate,
                    replace_loops(stmt.block)
                )
            else:
                return stmt
        
        new_body = replace_loops(body)
        ...
        updated_mod[func_name] = new_func
...
# Apply the pass
transformed_mod = FuseIdenticalLoops()(mod)
