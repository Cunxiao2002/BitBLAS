...
class Module:
    ...
    def fused_gemm_0_gemm_1(A: R.Tensor((512, 128), dtype="float16"), B: R.Tensor((128, 512), dtype="float16"), D: R.Tensor((512, 128), dtype="float16")) -> R.Tensor((512, 128), dtype="float16"):
        R.func_attr({"Primitive": 1})
        cls = Module
        with R.dataflow():
            C = R.call_tir(cls.gemm_0, (A, B), out_sinfo=R.Tensor((512, 512), dtype="float16"))
            gv = R.call_tir(cls.gemm_1, (C, D), out_sinfo=R.Tensor((512, 128), dtype="float16"))
            R.output(gv)
        return gv
    ...
    def main(A: R.Tensor((512, 128), dtype="float16"), B: R.Tensor((128, 512), dtype="float16"), D: R.Tensor((512, 128), dtype="float16"), E: R.Tensor((512, 128), dtype="float16")) -> R.Tensor((512, 128), dtype="float16"):
        cls = Module
        with R.dataflow():
            gv: R.Tensor((512, 128), dtype="float16") = cls.fused_gemm_0_gemm_1(A, B, D)
            R.output(gv)
        return gv