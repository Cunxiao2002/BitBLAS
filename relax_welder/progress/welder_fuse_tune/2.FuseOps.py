...
class Module:
    ...
    def fused_matmul_relu(x: R.Tensor((128, 128), dtype="float32"), w: R.Tensor((128, 128), dtype="float32")) -> R.Tensor((128, 128), dtype="float32"):
        R.func_attr({"Primitive": 1})
        cls = Module
        with R.dataflow():
            lv0 = R.call_tir(cls.matmul, (x, w), out_sinfo=R.Tensor((128, 128), dtype="float32"))
            gv = R.call_tir(cls.relu, (lv0,), out_sinfo=R.Tensor((128, 128), dtype="float32"))
            R.output(gv)
        return gv
    ...
    def main(x: R.Tensor((128, 128), dtype="float32"), w: R.Tensor((128, 128), dtype="float32")) -> R.Tensor((128, 128), dtype="float32"):
        cls = Module
        with R.dataflow():
            gv: R.Tensor((128, 128), dtype="float32") = cls.fused_matmul_relu(x, w)
            R.output(gv)
        return gv