...
class MyModule:
    ...
    @R.function
    def main(A: R.Tensor((512, 128), "float16"),
            B: R.Tensor((128, 512), "float16"),
            D: R.Tensor((512, 128), "float16"),
            F: R.Tensor((128, 128), "float16"),) -> R.Tensor((512, 128), "float16"):
            with R.dataflow():
                C = R.call_tir(MyModule.gemm_0, (A, B), out_sinfo=R.Tensor((512, 512), "float16"))
                E = R.call_tir(MyModule.gemm_1, (C, D), out_sinfo=R.Tensor((512, 128), "float16"))
                G = R.call_tir(MyModule.gemm_2, (E, F), out_sinfo=R.Tensor((512, 128), "float16"))
                R.output(G)
            return G

write_mod(MyModule, log_path, "module_init")
...
with transform.PassContext():
    ...
    write_mod(relax_mod, log_path, "FuseTIR_new")
