...
@T.prim_func
def gemm(A: T.Buffer((512, 128), "float16"), B: T.Buffer((128, 512), "float16"), C: T.Buffer((512, 512), "float16"), D: T.Buffer((512, 128), "float16"), E: T.Buffer((512, 128), "float16")):
    T.func_attr({"dlight.tensorcore_prenormlized": T.bool(True)})
    # with T.block("root"):
    ...
    D_reindex_shared_dyn = T.alloc_buffer((1, 512, 128), "float16", scope="shared.dyn") 
    D_reindex_shared_dyn_warp = T.alloc_buffer((1, 32, 8, 32, 8), "float16", scope="warp")
    E_reindex_shared_dyn = T.alloc_buffer((1, 512, 128), "float16", scope="shared.dyn")
    E_reindex_shared_dyn_warp = T.alloc_buffer((1, 32, 8, 32, 8), "float16", scope="warp")
                    ...
                    for ax2_0_2 in T.thread_binding(4, thread="threadIdx.z"):
                        ...
                        for ax0_0, ax1_0 in T.grid(1, 8):
                            with T.block("C_reindex_shared.dyn_warp_o"):
                                ...
                                C_1 = T.match_buffer(C_reindex_shared_dyn[v0_o, v1_o * 16:v1_o * 16 + 16, v2_o * 16:v2_o * 16 + 16], (16, 16), "float16", strides=("C_s0", "C_s1"), scope="shared.dyn", offset_factor=1)
                                for tx in T.thread_binding(32, thread="threadIdx.x"):
                        ...
                        for ax1_0_3_init, ax2_0_3_init in T.grid(1, 2):
                            with T.block("gemm_o_init_3"):
                                v0_o = T.axis.spatial(1, ax0)
                                v1_o = T.axis.spatial(32, ax1_0_0_ax2_0_0_fused + ax1_0_2 + ax1_0_3_init)
                                v2_o = T.axis.spatial(8, ax2_0_2 * 2 + ax2_0_3_init)
                                T.reads()
                                T.writes(E_reindex_shared_dyn_warp[0, v1_o, v2_o, 0:32, 0:8])
                                with T.block("gemm_init_o_3"):
                                    v1_i_init_o = T.axis.spatial(1, 0)
                                    v2_i_init_o = T.axis.spatial(1, 0)
                                    T.reads()
                                    T.writes(E_reindex_shared_dyn_warp[0, v1_o, v2_o, 0:32, 0:8])
                                    E_warp = T.match_buffer(E_reindex_shared_dyn_warp[0, v1_o, v2_o, 0:32, 0:8], (32, 8), "float16", scope="warp", offset_factor=1)
                                    for tx in T.thread_binding(32, thread="threadIdx.x"):
                                        T.mma_fill("float16", 8, E_warp.data, E_warp.elem_offset)
                            ...
                            for ax0_ax1_ax2_fused_0 in T.thread_binding(1, thread="threadIdx.y"):
                                for ax0_ax1_ax2_fused_1 in T.thread_binding(4, thread="threadIdx.z"):
                                    for ax0_ax1_ax2_fused_2 in T.unroll(16, annotations={"pragma_unroll_explicit": 0}):
                                        for ax0_ax1_ax2_fused_3 in T.thread_binding(32, thread="threadIdx.x"):
                                            for ax0_ax1_ax2_fused_4 in T.vectorized(8):
                                                with T.block("D_reindex_shared.dyn"):
                                                    v0 = T.axis.spatial(1, 0)
                                                    ...
                                                    T.reads(D[v1, v2])
                                                    T.writes(D_reindex_shared_dyn[v0, v1, v2])
                                                    T.block_attr({"buffer_dim_align": [[0, 1, 16, 8]], "permuted_layout": 0})
                                                    D_reindex_shared_dyn[v0, v1, v2] = D[v1, v2]
                            for ax3_0_1 in range(8):
                                for ax0_0, ax1_0 in T.grid(1, 1):
                                    with T.block("C_reindex_shared.dyn_warp_o"):
                                        v0_o = T.axis.spatial(1, 0)
                                        ...
                                        T.reads(C_reindex_shared_dyn[v0_o, v1_o * 16:v1_o * 16 + 16, v2_o * 16:v2_o * 16 + 16])
                                        T.writes(C_reindex_shared_dyn_warp[v0_o, v1_o, v2_o, 0:32, 0:8])
                                        T.block_attr({"permuted_layout": 0})
                                        warp = T.match_buffer(C_reindex_shared_dyn_warp[v0_o, v1_o, v2_o, 0:32, 0:8], (32, 8), "float16", scope="warp", offset_factor=16)
                                        shared = T.match_buffer(C_reindex_shared_dyn[v0_o, v1_o * 16:v1_o * 16 + 16, v2_o * 16:v2_o * 16 + 16], (16, 16), "float16", strides=("shared_s0", "shared_s1"), scope="shared.dyn", offset_factor=16)
                                        for tx in T.thread_binding(32, thread="threadIdx.x"):
                                            T.ptx_ldmatrix("float16", T.bool(False), 4, ".b16", warp.data, warp.elem_offset + 8 * tx, T.tvm_access_ptr(T.type_annotation("float16"), shared.data, shared.elem_offset, shared.strides[0] * 16, 1), shared.strides[0] * (tx % 16) + 8 * (tx // 16))
                                for ax0_0, ax1_0 in T.grid(1, 2):
                                    with T.block("D_reindex_shared.dyn_warp_o"):
                                        v0_o = T.axis.spatial(1, 0)
                                        v1_o = T.axis.spatial(32, ax3_0_0 * 8 + ax3_0_1 + ax0_0)
                                        v2_o = T.axis.spatial(8, ax2_0_2 * 2 + ax1_0)
                                        T.reads(D_reindex_shared_dyn[v0_o, v1_o * 16:v1_o * 16 + 16, v2_o * 16:v2_o * 16 + 16])
                                        T.writes(D_reindex_shared_dyn_warp[v0_o, v1_o, v2_o, 0:32, 0:8])
                                        T.block_attr({"permuted_layout": 0})
                                        warp = T.match_buffer(D_reindex_shared_dyn_warp[v0_o, v1_o, v2_o, 0:32, 0:8], (32, 8), "float16", scope="warp", offset_factor=16)
                                        shared = T.match_buffer(D_reindex_shared_dyn[v0_o, v1_o * 16:v1_o * 16 + 16, v2_o * 16:v2_o * 16 + 16], (16, 16), "float16", strides=("shared_s0", "shared_s1"), scope="shared.dyn", offset_factor=16)
                                        for tx in T.thread_binding(32, thread="threadIdx.x"):
                                            T.ptx_ldmatrix("float16", T.bool(True), 4, ".b16", warp.data, warp.elem_offset + 8 * tx, T.tvm_access_ptr(T.type_annotation("float16"), shared.data, shared.elem_offset, shared.strides[0] * 16, 1), shared.strides[0] * (tx % 16) + 8 * (tx // 16))
                                for ax1_0_3, ax2_0_3 in T.grid(1, 2):
                                    with T.block("gemm_o_update_1"):
                                        ...
                                        T.reads(E_reindex_shared_dyn_warp[0, v1_o, v2_o, 0:32, 0:8], C_reindex_shared_dyn_warp[0, v1_o, v3_o, 0:32, 0:8], D_reindex_shared_dyn_warp[0, v3_o, v2_o, 0:32, 0:8])
                                        T.writes(E_reindex_shared_dyn_warp[0, v1_o, v2_o, 0:32, 0:8])
                                        with T.block("gemm_o_1"):
                                            ...
                                            T.reads(E_reindex_shared_dyn_warp[0, v1_o, v2_o, 0:32, 0:8], C_reindex_shared_dyn_warp[0, v1_o, v3_o, 0:32, 0:8], D_reindex_shared_dyn_warp[0, v3_o, v2_o, 0:32, 0:8])
                                            T.writes(E_reindex_shared_dyn_warp[0, v1_o, v2_o, 0:32, 0:8])
                                            C_1 = T.match_buffer(C_reindex_shared_dyn_warp[0, v1_o, v3_o, 0:32, 0:8], (32, 8), "float16", scope="warp", offset_factor=16)
                                            D_1 = T.match_buffer(D_reindex_shared_dyn_warp[0, v3_o, v2_o, 0:32, 0:8], (32, 8), "float16", scope="warp", offset_factor=16)
                                            E_1 = T.match_buffer(E_reindex_shared_dyn_warp[0, v1_o, v2_o, 0:32, 0:8], (32, 8), "float16", scope="warp", offset_factor=16)
                                            for tx in T.thread_binding(32, thread="threadIdx.x"):
                                                T.ptx_mma("float16", "m16n8k16", "row", "col", "fp16", "fp16", "fp16", C_1.data, C_1.elem_offset + tx * 8, D_1.data, D_1.elem_offset + tx * 8, E_1.data, E_1.elem_offset + tx * 8, T.bool(False))
                                                T.ptx_mma("float16", "m16n8k16", "row", "col", "fp16", "fp16", "fp16", C_1.data, C_1.elem_offset + tx * 8, D_1.data, D_1.elem_offset + tx * 8 + 4, E_1.data, E_1.elem_offset + tx * 8 + 4, T.bool(False))
                        for ax0_0, ax1_0 in T.grid(1, 2):
                            with T.block("E_reindex_shared.dyn_warp_o"):
                                v0_o = T.axis.spatial(1, 0)
                                v1_o = T.axis.spatial(32, ax1_0_0_ax2_0_0_fused)
                                v2_o = T.axis.spatial(8, ax2_0_2 * 2 + ax1_0)
                                T.reads(E_reindex_shared_dyn_warp[v0_o, v1_o, v2_o, 0:32, 0:8])
                                T.writes(E_reindex_shared_dyn[v0_o, v1_o * 16:v1_o * 16 + 16, v2_o * 16:v2_o * 16 + 16])
                                E_warp = T.match_buffer(E_reindex_shared_dyn_warp[v0_o, v1_o, v2_o, 0:32, 0:8], (32, 8), "float16", scope="warp", offset_factor=1)
                                E_1 = T.match_buffer(E_reindex_shared_dyn[v0_o, v1_o * 16:v1_o * 16 + 16, v2_o * 16:v2_o * 16 + 16], (16, 16), "float16", strides=("E_s0", "E_s1"), scope="shared.dyn", offset_factor=1)
                                for tx in T.thread_binding(32, thread="threadIdx.x"):
                                    T.mma_store("float16", 16, 16, T.tvm_access_ptr(T.type_annotation("float16"), E_1.data, E_1.elem_offset, E_1.strides[0] * 16, 2), E_warp.data, E_warp.elem_offset, E_1.strides[0])
                    ...
                    for ax0_ax1_ax2_fused_0 in T.unroll(32, annotations={"pragma_unroll_explicit": 0}):
                        for ax0_ax1_ax2_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                            for ax0_ax1_ax2_fused_2 in T.vectorized(8):
                                with T.block("E_reindex_shared.dyn"):
                                    v0 = T.axis.spatial(1, 0)
                                    ...
                                    T.reads(E_reindex_shared_dyn[v0, v1, v2])
                                    T.writes(E[v1, v2])
                                    E[v1, v2] = E_reindex_shared_dyn[v0, v1, v2]
...
def find_block(stmt, block_name):
    """Find a block with the given name in the statement."""
    found_block = [None]
    
    def visitor(stmt):
        if isinstance(stmt, tvm.tir.Block) and stmt.name_hint == block_name:
            found_block[0] = stmt
        return None
...
module = tvm.build(gemm, target=target, name="fused_gemm")
